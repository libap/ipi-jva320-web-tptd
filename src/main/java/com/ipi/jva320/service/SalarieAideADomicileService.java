 ...
 * en créer, leur ajouter des congés (ajouteConge()), et les mettre à jour à la
 * clôture de mois et d'année.
 */
@Service
public class SalarieAideADomicileService {
     ...
     * (l'objectif est d'obliger les salariés à lisser leurs congés sur l'année,
     * mais quand même leur permettre de
    ...
    /**
     * Calcule les jours de congés à décompter (par
     * calculeJoursDeCongeDecomptesPourPlage()),
     * et si valide (voir plus bas) les décompte au salarié et le sauve en base de
     * données
     * 
    ...
    /**
     * Clôture le mois en cours du salarie donné (et fait les calculs requis pour sa
     * feuille de paie de ce mois) :
     * (pas forcément en cours, par exemple en cas de retard, vacances de
     * l'entreprise)
     * Met à jour les jours travaillés (avec ceux donnés) et congés payés acquis
     * (avec le nombre acquis par mois, qu'on suppose constant de 2.5) de l'année N
     * (le décompte d ceux de l'année N-1 a par contre déjà été fait dans
     * ajouteConge()).
    ...
    /**
     * Clôture l'année donnée. Il s'agit d'une année DE CONGES donc du 1er juin au
     * 31 mai.
